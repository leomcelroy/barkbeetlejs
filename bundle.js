import { svg, html, render } from 'https://cdn.pika.dev/lit-html@^1.1.2';
...
const connect_points = (pointsArray) => {
  ...
  pointsArray.forEach((point, index) => {
    if (index === 0) {
      ...
      lines.push(line$1(lastPoint, point));
      lastPoint = point;
    }
  });
...
const originPoint = ({contours, origin}) => {
    ...
    if (v.type !== "drill") {
      ...
      let cross = svg`
        <g  
          ...
          fill="none"
        >
          <polyline points="${x},${y + 5} ${x},${y - 5}"/>
          <polyline points="${x - 5},${y} ${x + 5},${y}"/>
        </g>`;
...
const suggest_bit = (state, { material, materialThickness }) => {
...
const recalculate = (state, { material, bitDiameter, flutes, rpm }) => {
...
function workerInternal() {
  self.onmessage = function(e) {
    ...
    // console.log(href); 
    if (href.includes("leomcelroy.com")) {
      href = "https://leomcelroy.com/content/projects/barkbeetlejs/";
    }
...
function clipperOffsetContourWorker(
  contour,
  offset,
  joints = 1,
  tolerance = 0.01
...
const depth_of_passes = (cutDepthArg, passDepthArg) => {
  ...
  return makeSeries(actualPassDepth, actualPassDepth, tempFloor).map(n => -n);
};
...
const pocketGcode = (toolpaths, params) => {
...
const createToolpaths$1 = async (contour, params) => {
  let outline;
...
const profileGcode = (toolpaths, params) => {
...
const profile = (contour, params) => {
  ...
  let toolpaths = createToolpaths$1(contour, params);
...
const drillGcode = (center, params) => {
  let units = "G21";
  // if (params.units === "in") units = "G20";
  // if (params.units === "mm") units = "G21";
...
const scale = (contours, xFactor, yFactor) => {
  contours = contours.map(contour => {
    return contour.map(l => {
      ...
      return line$1(newOrigin, newEnd);
    })
  });
...
const dispatch$1 = (action, args = {}, rerender = true) => {
  // console.log(action);
  //and update
  action = action.split(" ");
  ...
  switch (action) {
    ...
    case "SUGGEST_BIT":
      ...
      break;
    case "DRAG_TARGET":
      if (document.getElementById(`tablerow:${state.toolpath_drag.target}`) !== null) {
        document.getElementById(`tablerow:${state.toolpath_drag.target}`).classList.remove("hoveredRow");
      }
      document.getElementById(`tablerow:${args.target}`).classList.add("hoveredRow");
    ...
    case "REORDER":
      ...
      let draggedToolpath = state.toolpaths[draggedIndex];
      let newToolpaths = state.toolpaths.filter(p => p.id !== dragged);
      let index = draggedIndex < targetIndex ? targetIndex - 1 : targetIndex;

      newToolpaths.splice(index, 0, draggedToolpath);
    ...
    case "RECALCULATE":
      ...
      break;
    case "SAVE":
      download(`${state.filename}.bbjs`, JSON.stringify(state));
      break;
    case "UPLOAD_BBJS":
      let contours = {};
      let idMap = {};
    ...
    case "SELECT_TOOLPATH":
      state.toolpaths[args.id].selected = !state.toolpaths[args.id].selected;
      break;
    ...
    case "DOWNLOAD_GCODE":
      ...
      download(`${state.filename}.gcode`, text.join("\n"));
      break;
    case "CHANGE_ORIGIN":
    ...
    case "SAVE_PARAMETERS":
      ...
      if (state.popUpType.type === "default") {
        ...
        if (state.popUpType.createOrEdit === "create") {
          ...
          profile(state.contours[og.sourceGeometryID], args.params)
            .then(p => {
              state.toolpaths[state.popUpType.edit] = {
                ...state.toolpaths[state.popUpType.edit],
                geometry: p,
        ...
        if (state.popUpType.createOrEdit === "create") {
          ...
          let og = state.toolpaths[state.popUpType.edit];
          pocket(state.contours[og.sourceGeometryID], args.params)
            .then(p => {
              state.toolpaths[state.popUpType.edit] = {
                ...state.toolpaths[state.popUpType.edit],
                geometry: p,
