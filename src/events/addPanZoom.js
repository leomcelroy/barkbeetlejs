...
export function addPanZoom(el) {
  ...
  const transformGroupsNoScale = () => el.querySelectorAll(".transform-group-no-scale");
  ...
  function setTransformNoScale(els) {
    els.forEach(el => {
  ...
  listen("mousemove", "", (e) => {
    ...
    setTransformNoScale(transformGroupsNoScale());
  })
  ...
  listen("mouseout", "", (evt) => {
    // TODO: should check if not in el anymore

    // if (evt.target.closest(".transform-group"))
    // mousedown = false;
  })
  ...
  listen("wheel", "", (e) => {
    ...
    const wheelFactor = 1.05;
    if (Math.sign(e.deltaY) < 0) scale *= wheelFactor;
    else scale /= wheelFactor;
